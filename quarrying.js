...
function clickElementWithDelay(xpath) {
    ...
    if (element) {
        ...
        setTimeout(function () {
            element.click();
            console.log('Clicked quarrying element');
        }, delay);
    } else {
...
function performClick() {
    var defaultXPath = '/html/body/div/div[1]/div[4]/div[2]/div[1]/p/div/a[4]';
...
function startQuarrying() {
    console.log('Quarrying task started');
    // Run the first click without delay
    performClick();
    // Set interval for subsequent clicks every 180 seconds (3 minutes)
    quarryingInterval = setInterval(performClick, 180000);
}

function stopQuarrying() {
    console.log('Quarrying task stopped');
    clearInterval(quarryingInterval); // Stop the quarrying task by clearing the interval
}
