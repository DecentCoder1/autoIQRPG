...
function clickElementWithDelay(xpath) {
    ...
    if (element) {
        ...
        setTimeout(function () {
            element.click();
            console.log('Clicked battling element');
        }, delay);
    } else {
...
function performClick() {
    var defaultXPath = '/html/body/div/div[1]/div[4]/div[2]/div[1]/p/div/a[1]';
...
function startBattling() {
    console.log('Battling task started');
    // Run the first click without delay
    performClick();
    // Set interval for subsequent clicks every 180 seconds (3 minutes)
    battlingInterval = setInterval(performClick, 180000);
}

function stopBattling() {
    console.log('Battling task stopped');
    clearInterval(battlingInterval); // Stop the battling task by clearing the interval
}
